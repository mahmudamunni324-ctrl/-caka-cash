...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .light-theme .enhanced-task-container {
            background: linear-gradient(135deg, #FFFFFF, #F8F9FA);
            border-color: #E9ECEF;
        }
        .dark-theme .enhanced-task-container {
            background: linear-gradient(135deg, #2D3748, #4A5568);
            border-color: #718096;
        }
        .enhanced-task-container:hover {
...
<body>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const firebaseConfig = {
            apiKey: "AIzaSyD-qr-MzT-X7lqevL_f8ZcnGd_1gaVX0NU",
  authDomain: "domok-3bf48.firebaseapp.com",
  databaseURL: "https://domok-3bf48-default-rtdb.firebaseio.com",
  projectId: "domok-3bf48",
  storageBucket: "domok-3bf48.firebasestorage.app",
  messagingSenderId: "905230448130",
  appId: "1:905230448130:web:58e40ab8606e56f533191b",
  measurementId: "G-LWD2X1D5NS"
};
        ...
        const handleClaimTask = async (e) => {
            if ((e.target.classList.contains('action-btn') || e.target.classList.contains('enhanced-action-btn')) && e.target.dataset.taskId) {
                ...
                setTimeout(async () => {
                    try {
                        ...
                        renderUI();
                    } catch (error) { tg.showAlert('Failed to claim bonus.'); e.target.disabled = false; e.target.textContent = '𝗖𝗟𝗔𝗜𝗠 𝗡𝗢𝗪'; }
                }, 5000);
            }
        };
        ...
        const renderDynamicTasks = () => {
            ...
            sectionTitle.textContent = '🎯 Complete Tasks & Earn More';
            ...
            if (appConfig.tasks) {
                ...
                tasks.forEach(task => {
                    ...
                    taskEl.className = 'enhanced-task-container';
                    
                    taskEl.innerHTML = `
                        <img class="enhanced-task-icon" src="${task.icon}" alt="${task.name}" onerror="this.src='https://via.placeholder.com/60x60/3498db/ffffff?text=📷'">
                        <div class="enhanced-task-content">
                            <div class="enhanced-task-title">${task.name}</div>
                            <div class="enhanced-task-description">${task.description || 'Complete this task to earn bonus TK.'}</div>
                            <div class="enhanced-task-reward">
                                <i class="fas fa-coins"></i>
                                Reward: ${task.reward} TK
                            </div>
                        </div>
                        <div class="enhanced-task-actions">
                            <button class="enhanced-action-btn open-task-btn" onclick="window.Telegram.WebApp.openLink('${task.url}')">
                                🔘 𝗢𝗣𝗘𝗡 🔘
                            </button>
                            ${isCompleted ? 
                                '<div class="task-completed-badge">✅𝗖𝗟𝗔𝗜𝗠𝗘𝗗</div>' : 
                                `<button class="enhanced-action-btn claim-task-btn" data-task-id="${task.key}" data-task-url="${task.url}">
                                    𝗖𝗟𝗔𝗜𝗠 𝗡𝗢𝗪
                                </button>`
                            }
                        </div>
                ...
                container.querySelectorAll('.claim-task-btn').forEach(btn => {
                    btn.addEventListener('click', handleClaimTask);
                });
            } else {
                container.innerHTML = `
                    <div class="enhanced-task-container" style="justify-content: center; text-align: center;">
                        <div>
                            <i class="fas fa-tasks" style="font-size: 3rem; color: var(--primary-color); margin-bottom: 15px;"></i>
                            <h3>No Tasks Available</h3>
                            <p>Check back later for more earning opportunities!</p>
                        </div>
                    </div>
                `;
            }
